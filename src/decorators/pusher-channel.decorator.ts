...
export type ChannelBuilderMiddleware<Req = any, Res = any> = (req: Req, res: Res, eventName: string) => string;

/**
 * Decorates a route (required having PusherEvent decorator) by providing the target channel the PusherEvent should send
 */
export function PusherChannel(channel: string | string[] | ChannelBuilderMiddleware): MethodDecorator {
  return (
