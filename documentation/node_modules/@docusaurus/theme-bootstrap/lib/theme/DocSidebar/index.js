...
const DocSidebarItem = ({
  ...
  if (item.type === 'category') {
    return item.items.length > 0 ? <div>
        <h4 className="ml-2">{item.label}</h4>
        {item.items.map(childItem => <DocSidebarItem key={childItem.label} item={childItem} onItemClick={onItemClick} />)}
      </div> : <></>;
  } else if (item.type === 'link') {
    return <NavItem>
        <Link key={item.label} className="sidebar-item m-4 text-white" to={item.href} {...isInternalUrl(item.href) ? {
        isNavLink: true,
...
const DocSidebar = ({
  ...
  return <div className={classNames('bg-info', styles.sidebar)}>
      <div className={classNames('text-white', {
      [styles.isOpen]: sidebarShown
    })}>
        <div className="d-flex w-100 justify-content-end mr-5">
          <Button color="secondary" onClick={handleSidebarToggle} className={classNames('mr-2', styles.sidebarFAB)}>
            <svg aria-label="Menu" xmlns="http://www.w3.org/2000/svg" height={24} width={24} viewBox="0 0 32 32" role="img" focusable="false">
              <title>Menu</title>
