...
function BlogTagsListPage(props) {
  ...
  const renderAllTags = () => <>
      {Object.keys(tags).map(tag => <Link href={tags[tag].permalink} key={tag} className="btn btn-primary list-inline-item my-2">
          {tags[tag].name}{' '}
          <span className="badge badge-light">{tags[tag].count}</span>
        </Link>)}
    </>;

  return <Layout title="Tags" description="Blog Tags">
